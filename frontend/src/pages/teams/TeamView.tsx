...
import { addMemberToTeam, removeMemberFromTeam } from 'src/api/tasks';
                ...
                {sortedMembers.map((m) => {
                    ...
                    return (
                        <div tw="flex items-center w-full bg-gray-100 rounded my-1">
                            ...
                            <Box
                                tw="ml-auto mr-2 text-sm text-white p-1 rounded"
                                sx={{ bgcolor: color }}
                            >
                                {m.role}
                            </Box>
                            {!props.noEditable && (m.role === Role.Normal || m.role === Role.PM) && (
                    ...
                    (addingMode ? (
                        <div tw="flex items-center w-full mt-6">
                            <FormControl size="small" tw="w-48">
                                ...
                                >
                                    {users!
                                        .filter((u) => !u.teamName && u.role === Role.Normal)
                                        .map((u, i) => (
